...
{
    // de reference the argument and compute the dot product into the array dot
...
int main(int argc, char *argv[])
{
    ...
    for (thread = 0; thread < thread_count; thread++) // manually start threads
    {
    ...
    for (thread = 0; thread < thread_count; thread++) // wait for completion
    ...
    free(thread_handles);
    for (int i = 0; i < n; i++) // compute summation of results
